async function sccyberCheckPremiumSeat(profile){
 try{
  ...
  var pr=await client.from('profiles').select('premium_enabled,username').eq('id',profile.supabaseUserId).single();
  if(pr.error||!pr.data||pr.data.premium_enabled!==true)return false;
  var username=String(pr.data.username||profile.username||'').toLowerCase();
  var lr=await client.from('learners').select('id,username,organisation_id,active,created_at').eq('username',username).single();
  ...
  var list=all.data||[];
  var index=list.findIndex(function(x){return String(x.username||'').toLowerCase()===username});
